...
// ---------------- 存储层 ----------------
// 若设置 DATABASE_URL 则使用 Postgres（持久、重启不丢）；否则回退本地 JSON 文件（本地开发用）。
let pool = null
let dbCache = null
...
// 启动时初始化存储：建表/建库并把数据载入内存缓存。
async function initStorage() {
  if (process.env.DATABASE_URL) {
    pool = new pg.Pool({ connectionString: process.env.DATABASE_URL, ssl: { rejectUnauthorized: false } })
    await pool.query(`CREATE TABLE IF NOT EXISTS erp_data (id INTEGER PRIMARY KEY, data JSONB NOT NULL)`)
    const r = await pool.query(`SELECT data FROM erp_data WHERE id = 1`)
    if (r.rows.length === 0) {
      const def = defaultDB()
      seedAdmins(def)
      await pool.query(`INSERT INTO erp_data (id, data) VALUES (1, $1)`, [JSON.stringify(def)])
      dbCache = def
    } else {
      dbCache = { ...defaultDB(), ...(r.rows[0].data || {}) }
    }
    console.log('[REP] 存储模式: Postgres (DATABASE_URL)')
  } else {
    fs.mkdirSync(DATA_DIR, { recursive: true })
    try {
      const raw = fs.readFileSync(DB_FILE, 'utf8')
      dbCache = { ...defaultDB(), ...JSON.parse(raw) }
    } catch {
      dbCache = defaultDB()
      seedAdmins(dbCache)
      persistNow(dbCache)
    }
    console.log(`[REP] 存储模式: 本地文件 (${DATA_DIR})`)
  }
  if (!dbCache.users || dbCache.users.length === 0) {
    seedAdmins(dbCache)
    await persistNow(dbCache)
  }
}
...
function persistNow(data) {
  if (pool) {
    return pool
      .query(`UPDATE erp_data SET data = $1 WHERE id = 1`, [JSON.stringify(data)])
      .catch((e) => console.error('[REP] DB 持久化失败:', e.message))
  }
...
initStorage()
  .then(() => {
    server.listen(PORT, () => {
      ...
      console.log(`[REP] 数据存储: ${pool ? 'Postgres (DATABASE_URL)' : DATA_DIR}`)
      console.log(`[REP] 默认管理员: wanghuizhen / wanghuizhen123 ， jinhuaqiang / jinhuaqiang123`)
    })
  })
  .catch((e) => {
    console.error('[REP] 存储初始化失败，无法启动:', e)
    process.exit(1)
  })
